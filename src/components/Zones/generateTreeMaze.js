export function generateTreeMaze(
  width = 120,
  height = 40,
  zone = 'Aries',
  fertileTiles = [],
  ...
  // Nueva función para saber si es agua
  const isWaterTile = (x, y) => {
    return waterTiles.some(tile => tile.x === x && tile.y === y);
  };
  ...
  for (let x = 0; x < width; x++) {
    if (
      isTileFertile(x, 0, fertileTiles) &&
      !isInsideHouse(x, 0, houses) &&
      !isWaterTile(x, 0)
    ) {
      trees.push({ x, y: 0, type: treeType, energy: Math.floor(Math.random() * 51) + 100 });
    }
    if (
      isTileFertile(x, height - 1, fertileTiles) &&
      !isInsideHouse(x, height - 1, houses) &&
      !isWaterTile(x, height - 1)
    ) {
  ...
  for (let y = 1; y < height - 1; y++) {
    if (
      !leftOpenings.includes(y) &&
      isTileFertile(0, y, fertileTiles) &&
      !isInsideHouse(0, y, houses) &&
      !isWaterTile(0, y)
    ) {
      trees.push({ x: 0, y, type: treeType, energy: Math.floor(Math.random() * 51) + 100 });
    }
    if (
      !rightOpenings.includes(y) &&
      isTileFertile(width - 1, y, fertileTiles) &&
      !isInsideHouse(width - 1, y, houses) &&
      !isWaterTile(width - 1, y)
    ) {
  ...
  for (let x = 4; x < width - 4; x += 4) {
    for (let y = 1; y < height - 1; y++) {
      ...
      if (
        y % 5 !== 0 &&
        isTileFertile(x, y, fertileTiles) &&
        !isInsideHouse(x, y, houses) &&
        !isWaterTile(x, y)
      ) {
  ...
  for (let y = 4; y < height - 4; y += 4) {
    for (let x = 1; x < width - 1; x++) {
      if (
        (x === 1 && leftOpeningsSet.has(y)) ||
        (x === width - 2 && rightOpeningsSet.has(y))
      ) continue;
      if (
        x % 5 !== 0 &&
        isTileFertile(x, y, fertileTiles) &&
